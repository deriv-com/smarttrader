...
const Header = (() => {
            ...
            Client.getAllLoginids().forEach((loginid) => {
                if (!Client.get('is_disabled', loginid) && Client.get('token', loginid)) {
                    ...
                    if (!currency) {
                        ...
                        header_deposit.attr('href', Url.urlForDeriv('redirect', 'action=add_account'));
                    }
    ...
    const displayAccountStatus = () => {
        BinarySocket.wait('get_account_status', 'authorize', 'landing_company').then(() => {
            ...
            const messages = {
                currency             : () => ({ key: 'currency', title: localize('Set account currency'), message: localize('Please set the currency of your account to enable trading.'), type: 'danger', button_text: 'Set currency', button_link: Url.urlForDeriv('redirect', 'action=add_account') }),
