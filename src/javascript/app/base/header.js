...
const {
  ...
  callLogoutSilent,
} = require("../../_common/authentication");
...
const Header = (() => {
  ...
  const bindSvg = () => {
    ...
    applyToAllElements("#header__notification-icon", (el) => {
      el.src = Url.urlForStatic(
        `${header_icon_base_path}ic-bell.svg?${process.env.BUILD_HASH}`
      );
    });

    applyToAllElements("#header__notification-empty-img", (el) => {
      el.src = Url.urlForStatic(
        `${header_icon_base_path}ic-box.svg?${process.env.BUILD_HASH}`
      );
    });

    applyToAllElements("#header__account-settings", (el) => {
      el.src = Url.urlForStatic(
        `${header_icon_base_path}ic-user-outline.svg?${process.env.BUILD_HASH}`
      );
    });
    ...
    applyToAllElements(".wallet-apps-logo", (el) => {
      el.src = Url.urlForStatic(
        `${wallet_header_icon_base_path}wallet-apps-logo.svg?${process.env.BUILD_HASH}`
      );
    });

    applyToAllElements(".deriv-com-logo", (el) => {
      el.src = Url.urlForStatic(
        `${wallet_header_icon_base_path}wallet-deriv-logo.svg?${process.env.BUILD_HASH}`
      );
    });
    ...
    applyToAllElements("#cashier-icon", (el) => {
      el.src = Url.urlForStatic(
        `${header_icon_base_path}ic-cashier.svg?${process.env.BUILD_HASH}`
      );
    });
  ...
  const bindPlatform = () => {
    ...
    // Mobile
    const mobile_platform_list = getElementById(
    ...
    const should_show_bots_when_logged_in = Client.isAccountOfType("virtual")
      ? !Client.isMultipliersOnly()
      : !Client.isMF() && !Client.isOptionsBlocked();
    const should_show_bots = is_logged_in
      ? should_show_bots_when_logged_in
      : !isEuCountry();
    ...
    Object.keys(platforms).forEach((key) => {
      ...
      const platform_div = createElement("a", {
        class: `platform__list-item ${
          key === "smarttrader" ? "platform__list-item--active" : ""
        }`,
        href: platform.link,
      });
      const platform_icon = createElement("img", {
        src: `${Url.urlForStatic(
          `${header_icon_base_path}${platform.icon}?${process.env.BUILD_HASH}`
        )}`,
        class: "platform__list-item-icon",
      });
      const platform_text_container = createElement("div", {
        class: "platform__list-item-text ",
      });
      const platform_name = createElement("div", {
        text: platform.name,
        class: "platform__list-item-name",
      });
      const platform_desc = createElement("div", {
        text: platform.desc,
        class: "platform__list-item-desc",
      });
      const icon_container = createElement("span", {
        class: "platform__list-item-icon-container",
      });
    ...
    const traders_hub_link = Url.urlForDeriv(
      "",
      `ext_platform_url=${ext_platform_url}`
    );
    const platform_dropdown_cta_container = createElement("div", {
      class: "platform__dropdown-cta",
    });
    const platform_dropdown_cta_link = createElement("a", {
      text: localize("Looking for CFDs? Go to Trader's hub"),
      class: " platform__dropdown-cta--link",
      href: traders_hub_link,
    });
  ...
  const bindClick = () => {
    ...
    mobile_menu_livechat.addEventListener("click", () => {
    ...
    // Notification Event
    const notification_bell = getElementById(
      "header__notification-icon-container"
    );
    const notification_container = getElementById(
    ...
    const mobile_platform_switcher_current = getElementById(
      "mobile__platform-switcher-current"
    );
    const mobile_platform_switcher = getElementById(
      "mobile__platform-switcher-expand"
    );
    const mobile_platform_switcher_dropdown = getElementById(
      "mobile__platform-switcher-dropdown"
    );
    const mobile_platform_switcher_active =
    ...
    const showMobilePlatformSwitcher = (shouldShow) => {
      if (shouldShow) {
        ...
        mobile_platform_switcher_dropdown.classList.add(
          mobile_platform_switcher_active
        );
      } else {
        mobile_platform_switcher.classList.remove("rotated");
        mobile_platform_switcher_dropdown.classList.remove(
          mobile_platform_switcher_active
        );
      }
    };
    ...
    mobile_platform_switcher_current.addEventListener("click", () => {
      if (
        mobile_platform_switcher_dropdown.classList.contains(
          mobile_platform_switcher_active
        )
      ) {
    ...
    const acc_switcher = Client.hasWalletsAccount()
      ? getElementById("wallet_switcher")
      : getElementById("acc_switcher");
    const account_switcher = Client.hasWalletsAccount()
      ? getElementById("wallet__switcher")
      : getElementById("account__switcher");
    const account_switcher_dropdown = Client.hasWalletsAccount()
      ? getElementById("wallet__switcher-dropdown")
      : getElementById("account__switcher-dropdown");
    const account_switcher_active = Client.hasWalletsAccount()
      ? "wallet__switcher-dropdown--show"
      : "account__switcher-dropdown--show";
    ...
    const has_real_account = all_login_ids.some(
      (loginid) => !/^VRT/.test(loginid)
    );
    const is_virtual =
      current_active_login && current_active_login.startsWith("VRTC");
    const add_account_text_normal = document.getElementById(
      "add-account-text-normal"
    );
    const add_account_text_eu_country = document.getElementById(
      "add-account-text-eu"
    );
    ...
    wallet_switcher_close.addEventListener("click", () =>
      showAccDropdown(false)
    );
    ...
    document.addEventListener("click", (event) => {
      // Platform Switcher
      if (
        !platform_switcher.contains(event.target) &&
  ...
  const logoutOnClick = async () => {
    ...
    // await callLogoutSilent();
    console.log("WHY");
    await onLogoutWithOauth();
    // onLogoutWithOauth();
  };
      ...
      Client.getAllLoginids().forEach((loginid) => {
        ...
        if (
          ...
          const icon = Url.urlForStatic(
            `${wallet_header_icon_base_path}ic-wallets-currency-${getIcon()}.svg?${
              process.env.BUILD_HASH
            }`
          );
          const combined_icon = Url.urlForStatic(
            `${wallet_header_icon_base_path}ic-wallets-combined-${getIcon()}.svg?${
              process.env.BUILD_HASH
            }`
          );
          const current_active_login = Client.get("loginid");
          ...
          const account = createElement("div", {
            class: `wallet__switcher-acc ${
              is_current ? "wallet__switcher-acc--active" : ""
            }`,
            "data-value": loginid,
          });
          const icon_container = createElement("div", {
            class: "wallet__switcher-icon--container",
          });
          const account_icon = createElement("img", {
            src: combined_icon,
            class: "wallet__switcher-icon--currency",
          });
          const account_content = createElement("div", {
            class: "wallet__switcher--content",
          });
          const account_text = createElement("span", {
            text: localize("Deriv Apps"),
          });
          const account_currency = createElement("span", {
            text: `${is_real ? currency : localize("Demo")} Wallet`,
          });
          const account_balance = createElement("span", {
            class: `wallet__switcher-balance account__switcher-balance-${loginid}`,
          });
          const demo_batch1 = createElement("span", {
            text: localize("Demo"),
            class: "wallet__header-demo-batch",
          });
          const demo_batch2 = createElement("span", {
            text: localize("Demo"),
            class: "wallet__header-demo-batch",
          });
  ...
  const loginIDOnClick = (e) => {
    e.preventDefault();
    const el_loginid = findParent(
      e.target,
      Client.hasWalletsAccount()
        ? "div.wallet__switcher-acc"
        : "div.account__switcher-acc"
    );
    const acc_switcher_active = el_loginid.classList.contains(
      "account__switcher-acc--active"
    );
    const wallet_switcher_active = el_loginid.classList.contains(
      "wallet__switcher-acc--active"
    );
    ...
    if (el_loginid && !(acc_switcher_active || wallet_switcher_active)) {
      ...
      const wallet_switcher_dropdown = getElementById(
        "wallet__switcher-dropdown"
      );
      const account_switcher_dropdown = getElementById(
        "account__switcher-dropdown"
      );
      wallet_switcher_dropdown.classList.remove(
        "wallet__switcher-dropdown--show"
      );
      account_switcher_dropdown.classList.remove(
        "account__switcher-dropdown--show"
      );
    }
  };
