...
const elementInnerHtml =
  require("../../../_common/common_functions").elementInnerHtml;
...
const commonTrading = (() => {
  ...
  const displayContractForms = (id, elements, selected) => {
    ...
    const contract_to_show = /^(callputequal)$/.test(selected)
      ? "risefall"
      : selected;
  ...
  const displayCommentPrice = (node, currency, type, payout, position) => {
    if (node && type && payout) {
      ...
      const comment = `${localize("Net profit")}: ${formatMoney(
        currency,
        profit
      )} | ${localize("Return")} ${return_percent.toFixed(1)}%`;
  ...
  const displayTooltip = () => {
    ...
    if (tip) {
      const market =
        ActiveSymbols.getSymbols()[Defaults.get(UNDERLYING)].market;
      ...
      tip.setAttribute(
        ...
        urlFor(
          ...
          `anchor=${map_to_section_id[market]}#range-of-markets`
        )
      );
    }
  };
