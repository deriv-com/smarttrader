...
const Purchase = () => {
  ...
  const isloading = () =>
    !data?.topAmount && !data?.middleAmount && !data?.bottomAmount;
  ...
  const displayCurrency = (referenceField) =>
    referenceField !== "-" ? data?.currency : "";
  ...
  const ButtonTooltipWrapper = (button, description) => {
    ...
    return (
      <Tooltip
        actionText="label"
        as="div"
        tooltipContent={<span className="purchase-tooltip">{description}</span>}
        tooltipPosition="top"
        variant="base"
      >
        {button}
  ...
  if (
    isloading() ||
    (!data?.showPurchaseResults && data?.isPurchaseFormDisabled)
  ) {
    return (
  ...
  if (!data?.showPurchaseResults && !data?.error) {
    return (
      <div
        className={`quill-purchase-section ${
          data?.isPurchaseFormDisabled && "disabled"
        }`}
      >
          ...
          <div className="purchase-box">
            <div className="purchase-header">
              <div className="purchase-icon-box">
                <div
                  className={`contract_heading ${data?.middleContractType}`}
                />
                <Text size="md" centered bold>
                  {" "}
              ...
              <div className="purchase-body-box">
                <div className="purchase-amount-box">
                  <div className="purchase-amount-info-box">
                    ...
                    <Text size="md" className={data.middleAmountClassname} bold>
                      {data?.middleAmount} {displayCurrency(data?.middleAmount)}
                    </Text>
                  </div>
                  <div className="purchase-amount-info-box">
                    <Text size="md">{localize("Multiplier")}:</Text>
                    <Text size="md" bold>
                      {data?.middleMultiplier}{" "}
                      {displayCurrency(data?.middleMultiplier)}
                    </Text>
                  </div>
                </div>
                <div className="purchase-btn-box">
                  {ButtonTooltipWrapper(
                    <Button
                      onClick={() => triggerClick("#purchase_button_middle")}
                      color="purchase"
                      size="lg"
                      label={localize("Purchase")}
                      fullWidth
                      disabled={data?.middlePurchaseDisabled}
                    />,
                    data?.middleDescription
                  )}
                </div>
              </div>
            </div>
          ...
          <>
            <div className="purchase-box">
              <div className="purchase-header">
                <div className="purchase-icon-box">
                  <div
                    className={`contract_heading ${data?.topContractType}`}
                  />
                  <Text size="md" centered bold>
                    {data?.topDisplayText}
                  </Text>
                ...
                <div className="purchase-body-box">
                  <div className="purchase-amount-box">
                    <div className="purchase-amount-info-box">
                      <Text size="md">{localize("Stake")}:</Text>
                      <Text size="md" className={data.topAmountClassname} bold>
                        {data?.topAmount} {displayCurrency(data?.topAmount)}
                      </Text>
                    </div>
                    <div className="purchase-amount-info-box">
                      ...
                      <Text
                        size="md"
                        className={data.topPayoutAmountClassname}
                        bold
                      >
                        {data?.topPayoutAmount}{" "}
                        {displayCurrency(data?.topPayoutAmount)}
                      </Text>
                    </div>
                  </div>
                  <div className="purchase-btn-box">
                    {ButtonTooltipWrapper(
                      <Button
                        onClick={() => triggerClick("#purchase_button_top")}
                        color="purchase"
                        size="lg"
                        label={localize("Purchase")}
                        fullWidth
                        disabled={data?.topPurchaseDisabled}
                      />,
                      data?.topDescription
                    )}
                  </div>
            ...
            <div className="purchase-box">
              <div className="purchase-header">
                <div className="purchase-icon-box">
                  <div
                    className={`contract_heading ${data?.bottomContractType}`}
                  />
                  <Text size="md" centered bold>
                    {" "}
                ...
                <div className="purchase-body-box">
                  <div className="purchase-amount-box">
                    <div className="purchase-amount-info-box">
                      ...
                      <Text
                        size="md"
                        className={data.bottomAmountClassname}
                        bold
                      >
                        {data?.bottomAmount}{" "}
                        {displayCurrency(data?.bottomAmount)}
                      </Text>
                    </div>
                    <div className="purchase-amount-info-box">
                      <Text size="md">{localize("Payout")}:</Text>
                      <Text
                        size="md"
                        className={data.bottomPayoutAmountClassname}
                        bold
                      >
                        {data?.bottomPayoutAmount}{" "}
                        {displayCurrency(data?.bottomPayoutAmount)}
                      </Text>
                    </div>
                  </div>
                  <div className="purchase-btn-box">
                    {ButtonTooltipWrapper(
                      <Button
                        onClick={() => triggerClick("#purchase_button_bottom")}
                        color="sell"
                        size="lg"
                        label={localize("Purchase")}
                        fullWidth
                        disabled={data?.bottomPurchaseDisabled}
                      />,
                  ...
                  {data?.error.code === "AuthorizationRequired" && (
                    <>
                      <Button
                        ...
                        onClick={() =>
                          (window.location.href = data?.error.signupUrl)
                        }
                      />
                      <Text size="sm">
                      ...
                      <Button
                        variant="tertiary"
                        size="lg"
                        label={localize("Log in here")}
                        onClick={() =>
                          triggerClick("#authorization_error_btn_login")
                        }
                      />
                    </>
                  )}
                      ...
                      <Text size="sm">
                        {parse(
                          localize(
                            ...
                            [
                              "$10,000.00",
                              `<a id='top_up_cashier_redirect' href=''>`,
                              "</a>",
                            ]
                          )
                        )}
      ...
      <div className="top-box">
        ...
        <div className="body-box">
          ...
          <div className="table-box">
            {data?.pr_tablePayoutValue && (
              <div className="table-item">
                ...
                <Text size="sm" centered>
                  {parseData(data.pr_tablePayoutValue)}
                </Text>
              </div>
            )}
            {data?.pr_tableCostValue && (
              <div className="table-item">
                ...
                <Text size="sm" centered>
                  {parseData(data.pr_tableCostValue)}
                </Text>
              </div>
            )}
            {data?.pr_tableProfitValue && data?.pr_showTableProfit && (
              <div className="table-item">
                ...
                <Text size="sm" centered>
                  {parseData(data.pr_tableProfitValue)}
                </Text>
              </div>
            )}
          </div>
          <div className="info-box">
            {data?.pr_barrier && (
              <Text size="sm">{parseData(data.pr_barrier)}</Text>
            )}
            {data?.pr_reference && (
              <Text size="sm">{parseData(data.pr_reference)}</Text>
            )}
            {data?.pr_showBtn && (
              <Button
                className="view-btn"
...
export const init = () => {
  ReactDOM.render(<Purchase />, getElementById("purchase_container"));
};
