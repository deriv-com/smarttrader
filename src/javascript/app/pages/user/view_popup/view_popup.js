...
const getCurrencyDisplayCode =
  require("../../../common/currency").getCurrencyDisplayCode;
const changePocNumbersToString =
  require("../../../common/request_middleware").changePocNumbersToString;
...
const ViewPopup = (() => {
  ...
  const responseContract = (response) => {
    if (
      ...
      Utility.isEmptyObject(response.proposal_open_contract)
    ) {
    ...
    if (
      !Utility.getPropertyValue(response, [
    ...
    is_sold_before_start =
      contract.sell_time && contract.sell_time < contract.date_start;
  ...
  const showContract = () => {
    ...
    containerSetText(
      ...
      ContractTypeDisplay()[contract.contract_type]
    );
    containerSetText(
  ...
  const update = async () => {
    ...
    if (is_sold_before_start) {
      ...
      containerSetText(
        "trade_details_purchase_time",
        epochToDateTime(contract.purchase_time),
        "",
        true
      );
      purchaseManager.set({
      ...
      containerSetText(
        ...
        epochToDateTime(contract.date_start),
        "",
        true
      );
      purchaseManager.set({
    ...
    if (contract.barrier_count > 1) {
      containerSetText(
        ...
        is_sold_before_start ? "-" : addComma(contract.high_barrier),
        "",
        true
      );
      containerSetText(
        "trade_details_barrier_low",
        is_sold_before_start ? "-" : addComma(contract.low_barrier),
        "",
        true
      );
      purchaseManager.set({
        cd_barrier: is_sold_before_start
          ? "-"
          : addComma(contract.high_barrier),
        cd_barrierLow: is_sold_before_start
          ? "-"
          : addComma(contract.low_barrier),
      });
      ...
      containerSetText(
        "trade_details_barrier",
        contract.entry_tick_time && is_sold_before_start
          ? "-"
          : barrier_prefix + formatted_barrier,
        "",
        true
      );
      ...
      if (
        Reset.isReset(contract.contract_type) &&
    ...
    let current_spot =
      contract.status === "sold" ? "" : contract.current_spot_display_value;
    let current_spot_time =
      contract.status === "sold" ? "" : contract.current_spot_time;
    ...
    if (current_spot_time) {
      ...
      containerSetText(
        ...
        epochToDateTime(current_spot_time)
      );
      purchaseManager.set({
    ...
    containerSetText(
      ...
      `${contract.transaction_ids.buy} (${localize("Buy")}) ${
        contract.transaction_ids.sell
          ? `<br>${contract.transaction_ids.sell} (${localize("Sell")})`
          : ""
      }`
    );
    containerSetText(
    ...
    purchaseManager.set({
      ...
      cd_indicativePrice: indicative_price
    ...
    if (is_ended && !contract.sell_price) {
      containerSetText(
        ...
        localize("Waiting for contract settlement."),
        { class: "pending" }
      );
      purchaseManager.set({
      ...
      containerSetText(
        ...
        `${formatMoney(
          ...
          contract.profit_percentage > 0 ? "+" : ""
        }${addComma(contract.profit_percentage, 2)}%)</span>`,
        { class: contract.profit >= 0 ? "profit" : "loss" }
      );
      purchaseManager.set({
        cd_profitLoss: `${formatMoney(
          contract.currency,
          contract.profit
        )}<span class="percent">(${
          contract.profit_percentage > 0 ? "+" : ""
    ...
    if (is_multiplier_contract) {
      const {
        cancellation: { ask_price: deal_cancellation_price = 0 } = {},
        profit,
      } = contract;
      const total_pnl = +profit - deal_cancellation_price;
      containerSetText(
        "trade_details_deal_cancellation",
        deal_cancellation_price
          ? formatMoney(contract.currency, deal_cancellation_price)
          : "-"
      );
      containerSetText(
        "trade_details_total_pnl",
        `${formatMoney(contract.currency, total_pnl)}${
          deal_cancellation_price
            ? `<span class="percent"> (${localize(
    ...
    if (!is_started) {
      ...
      containerSetText(
        ...
        localize("Contract has not started yet")
      );
      purchaseManager.set({
        cd_entrySpot: "-",
      ...
      if (contract.entry_spot > 0) {
        // only show entry spot if available and contract was not sold before start time
        containerSetText(
          "trade_details_entry_spot > span",
          is_sold_before_start ? "-" : contract.entry_spot_display_value
        );
        purchaseManager.set({
          cd_entrySpot: is_sold_before_start
            ? "-"
            : contract.entry_spot_display_value,
        });
      }
      containerSetText(
        "trade_details_message",
        contract.validation_error && !is_unsupported_contract
    ...
    if (is_digit) {
      if (!chart_started && contract.entry_tick_time) {
        ...
        DigitDisplay.initTable(
          id_tick_chart,
          DigitDisplay.calculateTableHeight(contract),
          contract
        );
  ...
  const updateTimers = () => {
    ...
    if (!is_started || contract.status !== "open" || remained < 0) {
      ...
      containerSetText(
        ...
        (days > 0
          ? `${days} ${days > 1 ? localize("days") : localize("day")}, `
          : "") +
          moment(remained * 1000)
            .utc()
            .format("HH:mm:ss")
      );
  ...
  const contractEnded = () => {
    ...
    if (contract.status === "sold") {
      ...
      containerSetText(
        ...
        epochToDateTime(contract.sell_time),
        "",
        true
      );
      purchaseManager.set({
    ...
    if (
      ...
      !Lookback.isLookback(contract.contract_type) &&
      (!contract.exit_tick_time ||
        contract.exit_tick_time > contract.date_start)
    ) {
      initAuditTable(0);
    }
  };
  ...
  const appendAuditLink = (element_id) => {
    const link = Utility.createElement("a", {
      class: "link-audit btn btn__small btn--secondary",
    });
  ...
  const initAuditTable = (show) => {
    ...
    const div = Utility.createElement("div", {
      id: "sell_details_audit",
      class: "gr-8 gr-12-p gr-12-m gr-no-gutter invisible",
    });
    const table = Utility.createElement("table", {
      id: "audit_header",
      class: "gr-12",
    });
    const tr = Utility.createElement("tr", { class: "gr-row" });
    const th_previous = Utility.createElement("th", {
      class: "gr-4 gr-4-t gr-4-p gr-2-m",
    });
    const link = Utility.createElement("a", { class: "previous-wrapper" });

    link.appendChild(
      Utility.createElement("span", { class: "previous align-self-center" })
    );
    link.appendChild(
      Utility.createElement("span", {
        class: "nowrap view-chart",
        text: localize("View chart"),
      })
    );
    link.addEventListener("click", () => {
    ...
    tr.appendChild(
      Utility.createElement("th", {
        class: "gr-4 gr-4-t gr-4-p gr-8-m",
        text: localize("Audit Page"),
      })
    );
    tr.appendChild(
      Utility.createElement("th", { class: "gr-4 gr-4-t gr-4-p gr-2-m" })
    );
  ...
  const map_contract_type = {
    ...
    "run(high|low)": "runs",
    "call|put": () =>
      contract.entry_tick === +contract.barrier ? "risefall" : "higherlower",
  };
  ...
  const showExplanation = (div) => {
    ...
    Object.keys(map_contract_type).some((type) => {
      if (new RegExp(type, "i").test(contract.contract_type)) {
        explanation_section +=
          typeof map_contract_type[type] === "function"
            ? map_contract_type[type]()
            : map_contract_type[type];
        return true;
      }
      return false;
    ...
    xhttp.onreadystatechange = function () {
      ...
      const div_response = Utility.createElement("div", {
        html: this.responseText,
      });
      const div_to_show = div_response.querySelector(`#${explanation_section}`);
      if (div_to_show) {
        div_to_show.classList.add(
          "align-start",
          "gr-padding-20",
          "explanation-section",
          "gr-parent"
    ...
    new Promise((resolve) => {
      ...
      array_audit_data.forEach((audit_data) => {
        ...
        auditData.push({
          ...
          color: colorClass,
          date:
            audit_data.epoch && !isNaN(audit_data.epoch)
              ? moment
                  .unix(audit_data.epoch)
                  .utc()
  ...
  const createAuditTable = (title) => {
    ...
    const fieldset = Utility.createElement("fieldset", {
      class: "align-start",
    });
    const table = Utility.createElement("table", {
      class: "gr-12 gr-centered gr-12-p gr-12-m",
    });
  ...
  const createAuditHeader = (table) => {
    ...
    const td_mid = Utility.createElement("td", {
      class: "gr-3 audit-table-head no-margin",
    });
    td_mid.appendChild(
      Utility.createElement("strong", { text: localize("Spot") })
    );

    const td_last = Utility.createElement("td", {
      class: "gr-4 audit-table-head no-margin",
    });
    td_last.appendChild(
      Utility.createElement("strong", { text: localize("Spot time (GMT)") })
    );
  ...
  const createAuditRow = (
    ...
    if (
      document.querySelector(`.audit-dates[data-value='${date}']`) &&
      !remark
    ) {
      return;
    }
    ...
    const tr = Utility.createElement("tr", { class: "gr-row" });
    const td_remark = Utility.createElement("td", {
      class: "gr-4 remark",
      text: remark || "",
    });
    const td_tick = Utility.createElement("td", {
      class: "gr-3 spot-value",
      text: tick_display || "",
    });
    const td_date = Utility.createElement("td", {
      class: "gr-4 audit-dates",
      "data-value": date,
  ...
  const populateAuditTable = (show_audit_table) => {
    if (!contract.tick_count) {
      ...
      parseAuditResponse(
        contract_starts.table,
        contract.audit_details.contract_start,
        "Start"
      ).then(() => {
        ...
        if (contract.audit_details.contract_end && contract.status !== "sold") {
          ...
          parseAuditResponse(
            contract_ends.table,
            contract.audit_details.contract_end,
            "End"
          ).then(() => {
      ...
      parseAuditResponse(
        ...
        contract.audit_details.all_ticks,
        "Details"
      ).then(() => {
  ...
  const makeTemplate = () => {
    ...
    const $sections = $("<div/>").append(
      $(
        '<div class="gr-row container"><div id="sell_details_chart_wrapper" class="gr-8 gr-12-p gr-12-m"></div><div id="sell_details_table" class="gr-4 gr-12-p gr-12-m"></div></div>'
      )
    );
    let [barrier_text, low_barrier_text] = localize(["Barrier", "Low barrier"]);
    if (Lookback.isLookback(contract.contract_type)) {
      [barrier_text, low_barrier_text] = Lookback.getBarrierLabel(
        contract.contract_type,
        contract.barrier_count
      );
    ...
    const should_show_entry_spot =
      !Lookback.isLookback(contract.contract_type) &&
      !/digit/i.test(contract.contract_type);
    const should_show_barrier = !/runhigh|runlow/i.test(contract.contract_type);
    $sections.find("#sell_details_table").append(
      $(
        `<table>
            <tr id="contract_tabs"><th colspan="2" id="contract_information_tab">${localize(
              "Contract information"
            )}</th></tr><tbody id="contract_information_content">
            ${createRow(
            ...
            ${
              !contract.tick_count && !is_multiplier_contract
                ? createRow(
            ...
            ${createRow(
              Callputspread.isCallputspread(contract.contract_type)
                ? localize("Maximum payout")
                : localize("Potential payout"),
              "",
              "trade_details_payout"
            )}
            ${
              multiplier && Lookback.isLookback(contract.contract_type)
                ? createRow(
                    localize("Multiplier"),
                    "",
                    "trade_details_multiplier"
            ...
            <th colspan="2" id="barrier_change" class="invisible">${localize(
              "Barrier change"
            )}</th>
            <tbody id="barrier_change_content" class="invisible"></tbody>
            <tr><th colspan="2" id="trade_details_current_title">${localize(
              "Current"
            )}</th></tr>
            ${createRow(
              localize("Spot"),
              "trade_details_spot_label",
              "trade_details_current_spot",
              0,
              "<span></span>"
            )}
            ${createRow(
            ...
            ${createRow(
              localize("Indicative"),
              "trade_details_indicative_label",
              "trade_details_indicative_price"
            )}
            ${createRow(
              localize("Potential profit/loss"),
              "trade_details_profit_loss_label",
              "trade_details_profit_loss"
            )}
            ${
              is_multiplier_contract
                ? createRow(
                    ...
                    "trade_details_deal_cancellation_label",
                    "trade_details_deal_cancellation"
                  )
                : ""
            }
    ...
    $sections
      .find("#sell_details_chart_wrapper")
      .html(
        $("<div/>", {
          id: contract.tick_count ? id_tick_chart : "analysis_live_chart",
    ...
    $container
      ...
      .append(
        $("<div/>", { id: "errMsg", class: `notice-msg ${hidden_class}` })
      )
      .append(
        $("<div/>", {
          id: "notice_ongoing",
          class: `fill-bg-color gr-padding-10 ${hidden_class}`,
          text: localize(
    ...
    purchaseManager.set({
      ...
      cd_payoutLabel: Callputspread.isCallputspread(contract.contract_type)
        ? localize("Maximum payout")
        : localize("Potential payout"),
  ...
  const createRow = (localized_label, label_id, value_id, is_hidden, value) =>
    `<tr${is_hidden ? ` class="${hidden_class}"` : ""}><td${
      label_id ? ` id="${label_id}"` : ""
    }>${localized_label}</td><td${value_id ? ` id="${value_id}"` : ""}>${
      value || ""
    }</td></tr>`;

  const epochToDateTime = (epoch) =>
    `${moment.utc(epoch * 1000).format("YYYY-MM-DD HH:mm:ss")} GMT`;
  ...
  const showMessagePopup = (localized_text, localized_title, msg_class) => {
    ...
    $con.append(
      $("<div/>", { id: wrapper_id }).append(
        $("<div/>", { class: msg_class, html: localized_text })
      )
    );
    ViewPopupUI.showInpagePopup(
  ...
  const sellSetVisibility = (show) => {
    ...
    if (show) {
      ...
      $sell_wrapper
        .setVisibility(1)
        .append(
          $("<div/>", { id: sell_wrapper_id }).append(
            $("<button/>", {
              id: sell_button_id,
              class: "btn btn__large btn--secondary",
              text: is_started ? localize("Sell at market") : localize("Sell"),
            })
          )
        );
      if (is_started) {
      ...
      purchaseManager.set({
        cd_sellLabel: is_started
          ? localize("Sell at market")
          : localize("Sell"),
      });
      ...
      $container
        ...
        .click((e) => {
          ...
          sellSetVisibility(false);
          const bid_price_number = parseFloat(
            contract.bid_price.replace(/,/g, "")
          ); // API request should not have comma
          BinarySocket.send({
            sell: contract_id,
            price: bid_price_number,
          }).then((response) => {
  ...
  const addSellNote = ($sell_wrapper) => {
    ...
    $sell_wrapper.find("#sell_at_market_wrapper").append(
      $("<div/>", { class: "note" })
    ...
    purchaseManager.set({
      cd_sellInfo: `<strong>${localize("Note")}</strong>: ${localize(
        "Contract will be sold at the prevailing market price when the request is received by our servers. This price may differ from the indicated price."
      )}`,
      cd_showSell: true,
    });
  ...
  const responseSell = (response) => {
    ...
    sellSetVisibility(false);
    if (is_sell_clicked) {
      const formatted_sell_price = formatMoney(
        contract.currency,
      ...
      containerSetText(
        ...
        `${localize("You have sold this contract at [_1] [_2]", [
          ...
          getCurrencyDisplayCode(contract.currency),
        ])}
                <br />
                ${localize(
      ...
      purchaseManager.set({
        cd_sellMsg: `${localize("You have sold this contract at [_1] [_2]", [
          formatted_sell_price,
          getCurrencyDisplayCode(contract.currency),
        ])}
                    <br />
